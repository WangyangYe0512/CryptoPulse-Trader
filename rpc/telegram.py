...
from rpc.constants import __version__, DUST_PER_COIN, Config
from rpc.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from rpc.exceptions import OperationalException, RPCException
from rpc.utils import chunks, plural, dt_from_ts, dt_humanize_delta, fmt_coin, fmt_coin2, format_date, round_value
from rpc.persistence import Trade
from rpc.simple_rpc import RPC, RPCHandler
from rpc.rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
...
class Telegram(RPCHandler):
    ...
    def _start_thread(self):
        ...
        """
        # 使用 daemon 线程，避免退出时阻塞整个进程（仍会在 cleanup() 中正常停止）
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            try:
                # Avoid indefinite blocking here
                fut.result(timeout=5)
            except Exception:
                pass
            # Join with timeout to avoid blocking program exit
            self._thread.join(timeout=5)
            if self._thread.is_alive():
                logger.warning("Telegram thread did not stop within timeout. Proceeding with shutdown.")
        except Exception as e:
            logger.error(f"清理 Telegram 失败: {e}")
    ...
    def _format_exit_msg(self, msg: RPCExitMsg) -> str:
        ...
        profit_prefix = "部分" if is_sub_trade else ""
        cp_extra = ""
        exit_wording = "已平仓" if is_fill else "平仓中"
        if is_sub_trade or is_final_exit:
            ...
            if is_final_exit:
                profit_prefix = "部分"
                cp_extra = (
                    f"*最终盈亏:* `{msg['final_profit_ratio']:.2%} "
        ...
        message = (
            ...
            f"*{f'{profit_prefix}盈亏' if is_fill else f'未实现{profit_prefix}盈亏'}:* "
    ...
    async def _status_table(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            await self._send_msg(f"❌ 状态表格生成失败: {str(e)}")
            return
        ...
        for i in range(0, messages_count):
            ...
            try:
                ...
                await self._send_msg(f"❌ 表格格式化失败: {str(e)}")
                return
            if show_total and i == messages_count - 1:
            ...
            try:
                ...
                traceback.print_exc()
                # 发送错误消息给用户
                await self._send_msg(f"❌ 消息发送失败: {str(e)}")
        ...
        if trade_id != "cancel":
            try:
                ...
                if trade_id.lower() == "all":
                    # 使用 all 模式
                    result = await loop.run_in_executor(None, safe_async_db(self._rpc._rpc_force_exit), "all")
                    
                    # 发送平仓结果消息
                    if isinstance(result, dict):
                        status_msg = result.get('status', '平仓完成')
                        if 'closed_trades' in result and result['closed_trades'] > 0:
                            msg = f"✅ {status_msg}\n"
                            msg += f"📊 平仓数量: {result['closed_trades']} 笔\n"
                            msg += f"💰 总盈亏: {result.get('total_profit', 0):.2f} USDT"
                            
                            if 'trades' in result:
                                msg += "\n\n📋 详细记录:"
                                for trade_info in result['trades'][:5]:  # 最多显示5笔
                                    profit = trade_info['profit']
                                    profit_emoji = "📈" if profit > 0 else "📉" if profit < 0 else "➡️"
                                    msg += f"\n{profit_emoji} {trade_info['pair']} (ID:{trade_info['trade_id']}): {profit:.2f} USDT"
                                
                                if len(result['trades']) > 5:
                                    msg += f"\n... 还有 {len(result['trades']) - 5} 笔"
                        else:
                            msg = f"ℹ️ {status_msg}"
                    ...
                    try:
                        ...
                        result = await loop.run_in_executor(None, safe_async_db(self._rpc._rpc_force_exit), trade_id_int)
                        ...
                        if isinstance(result, dict):
                            ...
                            if result.get('profit') is not None:
                                ...
                                profit_emoji = "📈" if profit > 0 else "📉" if profit < 0 else "➡️"
                                msg = f"✅ {status_msg}\n"
                                msg += f"{profit_emoji} {result.get('pair', '')} (ID:{trade_id}): {profit:.2f} USDT"
                            else:
                                msg = f"ℹ️ {status_msg}"
                            
                            await self._send_msg(msg)
                        else:
                            await self._send_msg(f"❌ 意外的返回结果: {result}")
                    
                    except ValueError:
                        await self._send_msg(f"❌ 错误: '{trade_id}' 不是有效的交易ID")
                    except Exception as ex:
                        await self._send_msg(f"❌ 平仓执行失败: {str(ex)}")
                
            except RPCException as e:
                await self._send_msg(f"❌ 平仓失败: {str(e)}")
            except Exception as e:
                await self._send_msg(f"❌ 系统错误: {str(e)}")
